...
- commitが `error: gpg failed to sign the data` で失敗した場合、署名鍵(1Password等)がロック中、設定ミス、鍵失効などいずれの理由であっても署名が行えない状態にある。diffの内容やstage漏れが原因と誤認せず、`git commit --no-gpg-sign` で1回だけ再試行してよい(fail-open)。これは「署名不能な状態でも無人実行を止めない」という意図的なリスク受容であり、原因の切り分け(ロックか設定ミスか)は行わない。再試行で成功した場合は無署名commitになった旨・原因を切り分けていないこと・GitHub上でunverified表示になることを完了報告に明記し、可能であれば署名鍵が復旧した後に対象commitを `git commit --amend -S`（複数commitなら `git rebase --exec`）で再署名するようユーザーへ促す。同じ引数の再試行でも失敗する場合は署名以外の原因を疑い、原因を報告して停止する
